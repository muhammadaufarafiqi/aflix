...
class _MovieDetailState extends State<MovieDetailScreen> {
  ...
  Future<void> _fetch() async {
    ...
    try {
      final m = await context.read<AuthProvider>().api.getMovieById(widget.movieId);
  ...
  Future<void> _checkFavStatus() async {
    ...
    final status = await context.read<AuthProvider>().api.checkFavoriteStatus(_movie!.id);
  ...
  Future<void> _toggleFavorite() async {
    ...
    if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(_isFavorited ? '❤️ Ditambahkan ke Favorit' : 'Dihapus dari Favorit'),
      backgroundColor: _isFavorited ? AppTheme.primary : Colors.grey[800],
  ...
  void _play(String? url) {
    if (url == null || url.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Video tidak tersedia'), backgroundColor: Colors.red));
      return;
    }
          ...
          SliverToBoxAdapter(child: Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Wrap(spacing: 12, runSpacing: 6, children: [
                ...
                if (m.duration != null)
                  _metaBadge(child: Row(mainAxisSize: MainAxisSize.min, children: [
              ...
              // ── Aksi Sekunder (Favorit & Download) ──
              Row(children: [
                ...
                Expanded(child: _actionBtn(
                  ...
                  onTap: () => ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Fitur unduhan segera hadir!'),
  ...
  Widget _metaBadge({required Widget child, bool border = false}) =>
      Container(
        padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
        decoration: BoxDecoration(
            color: border ? Colors.transparent : AppTheme.card,
            borderRadius: BorderRadius.circular(6),
            border: border ? Border.all(color: AppTheme.textSecondary) : null),
        child: child,
      );
